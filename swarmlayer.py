...
from tools import CausalConv3d, CausalConv2d


class SwarmConvLSTMCell(nn.Module):

    def __init__(self, n_in, n_out, n_dim=2, pooling='CAUSAL', kernel=None):
        """
        ...
        if kernel is None:
            if n_dim==3:
                ...
                self.Wih = nn.Conv3d( n_in, 4 * n_out, (1,1,1), bias=True)
                self.Whh = nn.Conv3d(n_out, 4 * n_out, (1,1,1), bias=False)
                self.Whp = nn.Conv3d(n_out, 4 * n_out, (1,1,1), bias=False)
            elif n_dim==2:
            ...
            if n_dim==3:
                # output is 4 time n_out because it will be split into
                # input, output, and forget gates, and cell input
                self.Wih = CausalConv3d( n_in, 4 * n_out, kernel, bias=True)
                self.Whh = CausalConv3d(n_out, 4 * n_out, kernel, bias=False)
                self.Whp = CausalConv3d(n_out, 4 * n_out, kernel, bias=False)
            elif n_dim==2:
                self.Wih = CausalConv2d( n_in, 4 * n_out, kernel, bias=True)
                self.Whh = CausalConv2d(n_out, 4 * n_out, kernel, bias=False)
                self.Whp = CausalConv2d(n_out, 4 * n_out, kernel, bias=False)
            else:
                raise ValueError("dim {} not supported".format(n_dim))
...
class SwarmLayer(nn.Module):
    ...
    def __init__(self,
        ...
        self.n_iter = n_iter
        self.cell = SwarmConvLSTMCell(n_in, n_hidden, n_dim=n_dim, pooling=pooling, kernel=kernel)

        self.n_dim = n_dim
        if n_dim==3:
            # an output feed forward layer after. Because channel_first is default, is is implemented by a 1-by-1 conv.
            self.ffwd = nn.Conv3d(2 * n_hidden, n_out, (1,1,1), bias=True)
        elif n_dim == 2:
